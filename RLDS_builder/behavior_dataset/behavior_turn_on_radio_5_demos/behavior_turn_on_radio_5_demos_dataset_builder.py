...
VAL_DATA_PATH = "/vision/u/wsai/data/behavior/videos/task-0000/observation.images.rgb.head"
SOURCE = "video"
...
class behavior_turn_on_radio_5_demos(MultiThreadedDatasetBuilder):
    ...
    def _split_paths(self):
        ...
        if TRAIN_DATA_PATH:
            paths["train"] = glob.glob(f"{TRAIN_DATA_PATH}/*.{suffix}")[:5]
            print(len(paths["train"]))
        if VAL_DATA_PATH:
            paths["val"] = glob.glob(f"{VAL_DATA_PATH}/*.{suffix}")[-5:]
            print(len(paths["val"]))
